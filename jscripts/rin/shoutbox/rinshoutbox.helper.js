...
function rsbshout(authData) {
	...
	userRef = new Firebase(''+authData.url+'/onlist/'+authData.uid+''),
	simplelist = new Firebase(''+authData.url+'/onlist2/'+authData.uid+''),
	presimplelist = new Firebase(''+authData.url+'/onlist2/'),
	onlist = new Firebase(''+authData.url+'/onlist'),
	bmlfb = new Firebase(''+authData.url+'/bml/U'+authData.uid+''),
	prebmlfb = new Firebase(''+authData.url+'/bml/'),
	flolfb = new Firebase(''+authData.url+'/flol/'+authData.uid+''),
	numusr = '';
	amOnline.on('value', function(snapshot) {
		if (snapshot.val()) {
			sluid = parseInt(authData.uid);
			userRef.onDisconnect().remove();
			simplelist.onDisconnect().update({status: '0'});
			userRef.set({name: ''+authData.user+'', id: ''+authData.uid+''});
			bmlfb.once("value", function(snapshot) {
				if (!snapshot.val()) {
					bmlfb.set({nicks: ''+authData.user+'', ban: 0, uid: ''+authData.uid+''});
				}
				else {
	...
	var noticefb = new Firebase(""+authData.url+"/notice");
			...
			if (notban) {
				if ($('#shout_text').attr('data-type')=='shout') {
					...
					if(msg == '' || msg == null) {
						...
						$('#shout_text').val('').focus();
						if ( /^\/me[\s]+(.*)$/.test(msg) ) {
							messagesRef.push({msg: msg.slice(4), uid: authData.uid, colorsht: colorshout, bold: fontbold, font: fontype, size: fontsize, avatar: authData.avatar, nick: authData.user, edt: '0', type: 'system', created: Firebase.ServerValue.TIMESTAMP});
							flolfb.set({created: Firebase.ServerValue.TIMESTAMP});
						}
						else {
							messagesRef.push({msg: msg, uid: authData.uid, colorsht: colorshout, bold: fontbold, font: fontype, size: fontsize, avatar: authData.avatar, nick: authData.user, edt: '0', type: 'shout', created: Firebase.ServerValue.TIMESTAMP});
							flolfb.set({created: Firebase.ServerValue.TIMESTAMP});
						}
						return false;
					}
					...
					if(edmsg == '' || edmsg == null){
						...
						refedt = new Firebase(""+authData.url+"/shout/"+id+"");
						refedt.update({msg: edmsg, edt: '1'});
						return false;
					}
				}
			}
	...
	function displayMsg(reqtype, message, username, uid, colorsht, font, size, bold, avatar, edt, type, key, created, ckold, cur){
	...
	function checkMsg(req, msg, nick, uid, colorsht, font, size, bold, avatar, edt, type, _id, created, ckold, cur) {
		var mtype = 'shout';

		if (req=='lognext' || req=='logback') {
			mtype = req;
		}
		displayMsg(mtype, msg, nick, uid, colorsht, font, size, bold, avatar, edt, type, _id, created, ckold, cur);
	};
	...
	function newmsg (snapshot,id) {
		...
		if (key!=id) {
			...
			checkMsg("msg", docs.msg, docs.nick, docs.uid, docs.colorsht, docs.font, docs.size, docs.bold, docs.avatar, docs.edt, docs.type, snapshot.key(), docs.created, 'new', 0);
		}
	}
	...
	messagesRef.orderByKey().limitToLast(parseInt(numshouts)).once('value', function (snapshot) {
		if (snapshot.val()) {
			...
			for (var i = docs.length-1; i >= 0; i--) {
				checkMsg("msg", docs[i].msg, docs[i].nick, docs[i].uid, docs[i].colorsht, docs[i].font, docs[i].size, docs[i].bold, docs[i].avatar, docs[i].edt, docs[i].type, docs[i]._id, docs[i].created, 'old', i);
			}
	...
	function updmsg(message, key, edt) {
		message = regexrin(escapeHtml(revescapeHtml(message)));
		setTimeout(function() {
			if(parseInt(actaimg)) {
				imgconv(count);
	...
	function logfunc() {
		...
		messagesRef.orderByKey().limitToLast(parseInt(numslogs)).once('value', function (snapshot) {
			if (snapshot.val()) {
				...
				docs = predocs.reverse();
				for (var i = docs.length-1; i >= 0; i--) {
					checkMsg("lognext", docs[i].msg, docs[i].nick, docs[i].uid, docs[i].colorsht, docs[i].font, docs[i].size, docs[i].bold, docs[i].avatar, docs[i].edt, docs[i].type, docs[i]._id, docs[i].created, 'old', i);
				}
			}
		});
	}
	...
	if ($.inArray(parseInt(authData.gid), authData.rsbmodgroups.split(',').map(function(modgrup){return Number(modgrup);}))!=-1) {
		($.fn.on || $.fn.live).call($(document), 'dblclick', '.msgShout', function (e) {
			var id = $(this).attr('data-ided');
			function edtfunc(msg){
		...
		($.fn.on || $.fn.live).call($(document), 'click', '#banunbutton', function (e) {
			...
			if ($('#banlselector option:selected').val()!=null) {
				...
				if (parseInt(uid)==parseInt(authData.uid)) {
					...
					messagesRef.push({msg: banlist_modmsglan, uid: authData.uid, colorsht: colorshout, bold: fontbold, font: fontype, size: fontsize, avatar: authData.avatar, nick: authData.user, edt: '0', type: 'system', created: Firebase.ServerValue.TIMESTAMP});
				}
			}
			$.modal.close();
		});
		...
		($.fn.on || $.fn.live).call($(document), 'click', '#sv_notice', function (e) {
			...
			messagesRef.push({msg: not_modmsglan, uid: authData.uid, colorsht: colorshout, bold: fontbold, font: fontype, size: fontsize, avatar: authData.avatar, nick: authData.user, edt: '0', type: 'system', created: Firebase.ServerValue.TIMESTAMP});
			$.modal.close();
		});
		...
		($.fn.on || $.fn.live).call($(document), 'click', '#prune_yes', function (e) {
			...
			setTimeout(function() {
				messagesRef.push({msg: shout_prunedmsglan, uid: authData.uid, colorsht: colorshout, bold: fontbold, font: fontype, size: fontsize, avatar: authData.avatar, nick: authData.user, edt: '0', type: 'system', created: Firebase.ServerValue.TIMESTAMP});
			},50);
			$.modal.close();
		});
	}
